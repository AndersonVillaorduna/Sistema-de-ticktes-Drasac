                ...
                <>
                  <div className="card p-5">
                    ...
                    <div className="grid grid-cols-2 gap-3 text-xs">
                      <div><span className="text-neutral-400">Prioridad:</span> <span className="font-semibold capitalize">{ticketDetail.prioridad}</span></div>
                      <div><span className="text-neutral-400">Usuario:</span> <span className="font-semibold">{ticketDetail.usuario_nombre}</span></div>
                      <div><span className="text-neutral-400">Creado:</span> <span className="font-semibold">{new Date(ticketDetail.created_at).toLocaleDateString('es-PE')}</span></div>
                      <div><span className="text-neutral-400">Técnico:</span> <span className="font-semibold">{ticketDetail.tecnico_nombre || 'Sin asignar'}</span></div>
                    </div>
                    <button onClick={() => navigate(`/admin/tickets/${ticketDetail.id}`)}
                      className="mt-4 w-full py-2.5 rounded-lg bg-primary/10 text-primary text-xs font-bold hover:bg-primary/20 transition-all">
                  ...
                  {ticketDetail.respuesta_ia && (
                    <div className="ai-card">
                      <div className="ai-head">
                        ...
                        <div>
                          <p className="ai-eyebrow"><Sparkles className="w-3 h-3" /> Clasificación IA</p>
                      ...
                      {ticketDetail.confianza_ia != null && (
                        <>
                          <div className="confidence-row"><span>Confianza</span><span>{ticketDetail.confianza_ia}%</span></div>
                          <div className="confidence-track"><div className="confidence-fill" style={{ width: `${ticketDetail.confianza_ia}%` }} /></div>
                        </>
                      )}
                    </div>
                  )}
                          ...
                          return (
                            <div key={com.id || idx} className="tl-item">
                              ...
                              <div className="tl-content">
                                <div className="tl-header"><strong>{com.usuario_nombre}</strong><span>{new Date(com.created_at).toLocaleTimeString('es-PE', { hour: '2-digit', minute: '2-digit' })}</span></div>
                                <p>{com.mensaje}</p>
                              </div>
                            </div>
                          );
